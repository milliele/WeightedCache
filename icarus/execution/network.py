...
class NetworkView(object):
	...
	def link_weight(self, u, v):
		...
		delay : float
			The link delay
		"""
		return self.model.link_weight[(u, v)]
...
class NetworkModel(object):
	...
	def __init__(self, topology, cache_policy, shortest_path=None):
		...
		self.link_weight = nx.get_edge_attributes(topology, 'util')
		# print self.link_weight.keys()
		if not topology.is_directed():
			for (u, v), link_weight in list(self.link_weight.items()):
				self.link_weight[(v, u)] = link_weight
		# print self.link_weight.keys()
...
class NetworkController(object):
	...
	def has_content(self, node):
		...
		"""
		if node in self.model.cache:
			cache_hit = self.model.cache[node].has(self.session['content'])
			if cache_hit:
				if self.session['log']:
					self.collector.cache_hit(node)
			else:
